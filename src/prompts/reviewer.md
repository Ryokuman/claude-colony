...
3. **이슈 등록 및 위험 보고 서식**
   ...
   **이 코드는 어디에 있고, 무엇을 하는 곳인가**:
   - 이 함수/모듈이 시스템에서 어떤 역할을 하는지 (어떤 기능의 어떤 부분인지)
   - 왜 존재하는지 (어떤 상위/하위 요구사항 때문에 필요한지)
   - 이 코드를 호출하거나 의존하는 곳 (호출 체인)

   **왜 문제인가**:
   이 버그가 유저/시스템에 미치는 실제 영향. "이 버그 때문에 X가 Y로 동작한다"
   ...
   **방안 B: [이름]**
   ...
   > **"이 코드는 어디에 있고, 무엇을 하는 곳인가"** — WHAT이 아니라 WHY를 쓴다. "콤마로 구분된 문자열을 배열로 변환한다"가 아니라, "Obsidian이 라벨을 YAML 텍스트로 저장하므로 deriveStatus()에서 사용하려면 배열 변환이 필요하다" 같이 쓴다.
   >
   > **"왜 문제인가"** — 이 버그가 존재하면 어떤 일이 벌어지는지. "Jira 이슈 상태가 항상 todo로 표시된다" 같이 유저가 체감하는 영향을 쓴다.
